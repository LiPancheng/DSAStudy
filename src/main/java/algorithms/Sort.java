 ...
 *
 * 对于标准库里的Arrays.sort()：
 * JDK6里对原始类型用的是快速排序，对于对象类型(Object[])，则使用归并排序。
...
public class Sort {
     ...
     *3.递归地把两边的子数列排序。
     *
     * 时间复杂度：最优 O(nlogn)，平均 O(nlogn)，最差 O(n^2)即每次基准取到最大或最小
     * 空间复杂度：最优 O(logn)
     * 稳定性：不稳定
     * 分类：比较排序
     */
     ...
     * 方式为对于以i为根节点的子二叉树，选择它与左右孩子 三个中最大的值作为根
     *
     * 时间复杂度： 最优 O(nlogn)，平均 O(nlogn)，最差 O(nlogn)
     * 稳定性: 不稳定
     * 分类：比较排序
    ...
    /*
     * 对于以i为根节点的子二叉树，选择它与左右孩子 三个中最大的值作为根
     */
